...
const { required, numeric, minLength, maxLength, helpers, between } = window.validators;
const phone = helpers.regex("phone", /^09\d/);
const nationalCodeChecker = (value) => {
    if (value.length === 0) {
        return true
    } else {
        const a = value.split('')
        ...
        const controlDigit = a[a.length - 1]
        let total = 0
